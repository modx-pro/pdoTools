...
class Fetch extends CoreTools
{
    ...
    // ДОБАВЬТЕ ЭТОТ БЛОК ДЛЯ ПРОВЕРКИ И ЗАМЕНЫ НЕСУЩЕСТВУЮЩИХ ПОЛЕЙ issues #368
    if (!empty($this->config['sortby'])) {
        $sortFields = array_map('trim', explode(',', $this->config['sortby']));
        $validSortFields = [];
        
        foreach ($sortFields as $field) {
            // Убираем псевдонимы и проверяем существование поля
            $cleanField = preg_replace('/^.*\./', '', $field); // убираем "alias."
            $cleanField = preg_replace('/\s+(ASC|DESC)$/i', '', $cleanField); // убираем направление сортировки
            
            if (array_key_exists($cleanField, $fields) || 
                strpos($field, 'TV') !== false || 
                strpos($field, '(') !== false) { // TV поля или функции
                $validSortFields[] = $field;
            } else {
                // Заменяем несуществующее поле на первичный ключ
                $this->modx->log(xPDO::LOG_LEVEL_WARN, '[pdoTools] Field "' . $field . '" not found in class "' . $this->config['class'] . '", using primary key instead');
                $validSortFields[] = $alias . '.' . $this->pk;
            }
        }
        
        if (!empty($validSortFields)) {
            $this->config['sortby'] = implode(',', $validSortFields);
        }
    }
    ...
    if (empty($tmp) || (is_string($tmp) && in_array(strtolower($tmp), ['resources', 'ids']))) {
        ...
        if (!empty($this->config['where'][$resources])) {
            ...
            // Priority list of fields for default sorting
            $priorityFields = [];
            $availableFields = array_keys($fields);

            // Приоритетные поля, которые могут существовать в разных классах
            $possibleFields = ['publishedon', 'createdon', 'menuindex', 'pagetitle', 'username', 'id'];

            foreach ($possibleFields as $field) {
                if (array_key_exists($field, $fields)) {
                    $priorityFields[] = $field;
                }
            } // ЗАКРЫВАЮЩАЯ СКОБКА ДЛЯ FOREACH - ЭТО БЫЛО ПРОПУЩЕНО

            // Если не нашли подходящих полей, используем первичный ключ
            if (empty($priorityFields)) {
                $priorityFields = [$this->pk];
            }
    
            // Find first available priority field or use primary key as fallback
            $defaultSortField = $this->pk;
            ...
            $tmp = [
                $alias . '.' . $defaultSortField => !empty($this->config['sortdir'])
                    ? $this->config['sortdir']
                    : 'ASC',
            ];
        }
