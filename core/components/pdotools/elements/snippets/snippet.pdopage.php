...
// Limit - НОВАЯ ЛОГИКА С limitOnPage
// Приоритет: limitOnPage > limit из запроса > limit из параметров
if (!empty($limitOnPage)) {
    // Если задан limitOnPage, используем его и игнорируем limit из запроса
    $scriptProperties['limit'] = (integer)$limitOnPage;
} else {
    // Старая логика для limit
...
// Если limit все еще не установлен, используем значение из параметров
if (empty($scriptProperties['limit']) && isset($limit)) {
    $scriptProperties['limit'] = (integer)$limit;
}
...
/** @var bool $showLog */
$showLog = !empty($showLog); // Инициализируем переменную ОЧЕНЬ СТАРАЯ ОШИБКА issues 351
...
if ($isAjax) {
    ...
    // Fix for outputSeparator in AJAX mode issues #354
    if (!empty($scriptProperties['outputSeparator']) && $page > 1) {
        $data['output'] = $scriptProperties['outputSeparator'] . $data['output'];
    }
